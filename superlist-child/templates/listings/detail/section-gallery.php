...
<?php if ( apply_filters( 'inventor_metabox_allowed', true, 'gallery', get_the_author_meta('ID') ) ): ?>
    ...
    <?php if (empty($gallery))  { // zig - check for attached images
        $media = get_attached_media( 'image' );
        if ((count($media) > 1) && !get_theme_mod( 'inventor_general_show_featured_image_in_gallery', false )) {
          //unset($media[$featured_image_id]);
          $gallery = $media;
        }
    } ?>
